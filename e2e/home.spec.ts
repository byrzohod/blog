...
test.describe('Subscribe Page', () => {
  test('should display subscribe options', async ({ page }) => {
    await page.goto('/subscribe');

    await expect(page.locator('h1')).toContainText('Subscribe');
    await expect(page.getByRole('heading', { name: /email subscription/i })).toBeVisible();
    await expect(page.getByRole('heading', { name: /rss feed/i })).toBeVisible();
  });

  test('should have email input with validation', async ({ page }) => {
    await page.goto('/subscribe');

    // Check that email input exists and is required
    const emailInput = page.getByLabel('Email');
    await expect(emailInput).toBeVisible();

    // Verify the email input has the correct type attribute
    await expect(emailInput).toHaveAttribute('type', 'email');

    // Verify the subscribe button is present and enabled
    const subscribeButton = page.locator('form').getByRole('button', { name: /subscribe/i });
...
test.describe('Archive Page', () => {
  test('should display archive page', async ({ page }) => {
    ...
    // Should show stats section with exact text match
